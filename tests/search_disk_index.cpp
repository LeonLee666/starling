...
int search_disk_index(
  ...
  for (size_t i = 0; i < original_query_num; i++) {
    ...
    std::memcpy(query + (i * (variant_num + 1)) * query_aligned_dim,
    ...
    for (size_t v = 1; v <= variant_num; v++) {
      ...
      std::memcpy(query + target_idx * query_aligned_dim,
                  original_query + i * query_aligned_dim,
                  query_aligned_dim * sizeof(T));
      ...
      if (std::is_same<T, float>::value) {
        // For float type: apply percentage-based noise (1-5%)
        std::uniform_real_distribution<float> noise_dist(-0.05f, 0.05f);
        float noise = noise_dist(gen);
        query[target_idx * query_aligned_dim + perturb_dim] = 
            static_cast<T>(static_cast<float>(original_value) * (1.0f + noise));
      } else {
        // For integer types: apply small additive noise (±1 or ±2)
        std::uniform_int_distribution<int> int_noise_dist(-2, 2);
        ...
        query[target_idx * query_aligned_dim + perturb_dim] = static_cast<T>(new_value);
      }
    }
  }
  
  // Update query count
  query_num = expanded_query_num;
  
  // Free original query array
  diskann::aligned_free(original_query);
  
  diskann::cout << "Query expansion complete: " << original_query_num 
                << " -> " << query_num << " queries" << std::endl;
  ...
  if (gt_file != std::string("null") && gt_file != std::string("NULL") &&
    ...
    // Expand ground truth to match expanded queries
    if (gt_num == original_query_num) {
      diskann::cout << "Expanding ground truth to match query expansion..." << std::endl;
      
      size_t original_gt_num = gt_num;
      size_t expanded_gt_num = gt_num * (variant_num + 1);
      unsigned* original_gt_ids = gt_ids;
      float* original_gt_dists = gt_dists;
      
      // Allocate new ground truth arrays
      gt_ids = new unsigned[expanded_gt_num * gt_dim];
      gt_dists = new float[expanded_gt_num * gt_dim];
      
      // Replicate each original ground truth (variant_num + 1) times
      for (size_t i = 0; i < original_gt_num; i++) {
        for (size_t v = 0; v < variant_num + 1; v++) {
          size_t target_idx = i * (variant_num + 1) + v;
          std::memcpy(gt_ids + target_idx * gt_dim,
                      original_gt_ids + i * gt_dim,
                      gt_dim * sizeof(unsigned));
          std::memcpy(gt_dists + target_idx * gt_dim,
                      original_gt_dists + i * gt_dim,
                      gt_dim * sizeof(float));
        }
      }
      
      // Free original ground truth arrays
      delete[] original_gt_ids;
      delete[] original_gt_dists;
      
      // Update ground truth count
      gt_num = expanded_gt_num;
      
      diskann::cout << "Ground truth expansion complete: " << original_gt_num 
                    << " -> " << gt_num << " entries" << std::endl;
    }
